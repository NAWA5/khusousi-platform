...
import { Search, Menu, User, ShoppingCart, Sun, Moon, ChevronDown, LayoutGrid, X } from 'lucide-react';
                    ...
                    <div className="flex flex-1 items-center justify-center gap-3">
                        <nav className="hidden lg:flex items-center gap-1 text-sm font-medium text-gray-700">
                            ...
                            <div className="relative">
                                ...
                                {isCoursesOpen && (
                                    <div className="absolute left-1/2 -translate-x-1/2 mt-3 w-64 bg-white border border-gray-100 rounded-xl shadow-lg">
                            ...
                            <Link href="/courses" className="px-3 py-2 hover:text-primary transition-colors rounded-lg flex items-center gap-1">
                                <LayoutGrid size={16} />
                                التصنيفات
                            </Link>
                        </nav>
                        ...
                        <div className="hidden md:flex flex-1 max-w-md relative">
                            <form onSubmit={handleSearch} className="relative w-full">
                                ...
                                />
                                <button type="submit" className="absolute right-1 top-1/2 -translate-y-1/2 p-1.5 bg-primary text-white border border-primary rounded-full hover:bg-primary/90 transition-colors shadow-sm">
                                    <Search size={18} />
                                </button>
                            </form>
                        </div>
                ...
                {isMenuOpen && (
                    <div className="lg:hidden mt-3 rounded-2xl border border-gray-100 bg-white shadow-lg p-4 space-y-3">
                        <div className="flex flex-col gap-2 text-sm font-medium text-gray-700">
                            <Link href="/" className="px-3 py-2 rounded-lg hover:bg-gray-50" onClick={() => setIsMenuOpen(false)}>الرئيسية</Link>
                                        ...
                                        {universities.map((uni) => (
                                            <li key={uni.href}>
                                                <Link
                                                    href={uni.href}
                                                    className="block px-4 py-2 hover:bg-gray-50"
                                                    onClick={() => { setIsMenuOpen(false); setIsCoursesOpen(false); }}
                                                >
                                                    {uni.name}
                                                </Link>
                                            </li>
                            ...
                            <Link href="/courses" className="px-3 py-2 rounded-lg hover:bg-gray-50 flex items-center gap-2" onClick={() => setIsMenuOpen(false)}>
                                <LayoutGrid size={16} />
                                التصنيفات
                            </Link>
                        </div>
