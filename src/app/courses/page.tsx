...
function CoursesContent() {
    ...
    const [selectedUniveristy, setSelectedUniversity] = useState<string[]>([]);
    ...
    const filteredCourses = MOCK_COURSES.filter(course => {
        const matchQuery = course.title.includes(query) || course.instructor.includes(query);
        const matchUni = selectedUniveristy.length === 0 || selectedUniveristy.includes(course.university);
        const matchCat = selectedCategory.length === 0 || selectedCategory.includes(course.category);
        // simple price logic mock
        ...
        <div className="flex flex-col md:flex-row gap-8">
            ...
            <aside className={`md:w-64 flex-shrink-0 ${isMobileFiltersOpen ? 'fixed inset-0 z-50 bg-white p-6 overflow-y-auto' : 'hidden md:block'}`}>
                <div className="flex items-center justify-between mb-6 md:hidden">
                    ...
                    <button onClick={() => setIsMobileFiltersOpen(false)} className="text-gray-500">إغلاق</button>
                </div>
                ...
                <div className="space-y-8">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {['جامعة الملك سعود', 'جامعة الإمام', 'جامعة الأميرة نورة', 'جامعة الملك فهد', 'جامعة الملك عبدالعزيز'].map((uni) => (
                                <label key={uni} className="flex items-center gap-2 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        className="rounded border-gray-300 text-primary focus:ring-primary"
                                        checked={selectedUniveristy.includes(uni)}
                                        onChange={() => toggleFilter(selectedUniveristy, setSelectedUniversity, uni)}
                                    />
            ...
            <main className="flex-1">
                ...
                {filteredCourses.length > 0 ? (
                    ...
                    <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
                        ...
                        <button
                            onClick={() => { setSelectedUniversity([]); setSelectedCategory([]); }}
